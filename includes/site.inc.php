<?php
  function retrieveSiteById($siteId) {
    ...
    try {
      ...
      if(!($stmt = $GLOBALS['___mysqli_ston']->prepare("SELECT city, address FROM site WHERE site_id = " . $siteId))) {
        $_SESSION['technical_error'] = "Sorry, a technical error has occured.";
        return;
      }
  ...
  function retrieveUsersBySite($siteId) {
    ...
    try {
      ...
      if(!($stmt = $GLOBALS['___mysqli_ston']->prepare("SELECT lastname, firstname, mail, role FROM user WHERE site_id =  ?"))) {
        $_SESSION['technical_error'] = "Sorry, a technical error has occured.";
        return;
      }
